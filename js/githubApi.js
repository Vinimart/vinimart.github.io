const { GithubApiView } = require("./GithubApiView");
...
class GetApi {
	...
	async githubFetch(repo, modal) {
		...
		const response = await fetch(`https://api.github.com/repos/${gitHubToken.userName}/${repo}?client_id=${gitHubToken.userName}&client_secret=${gitHubToken.gitHubToken}`);
		...
		if (response.status === 200) {
			...
			this.view.renderPortfolioError(response.status, gitHubToken.userName);
		}
	}
